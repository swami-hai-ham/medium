...
import { signupInput } from "@swamihaihum/medium-common";
    ...
    <div className="relative flex flex-col justify-center items-center h-screen w-full ">
      <div className="flex flex-col md:flex-row justify-center items-center h-full w-full bg-[url('/bgart.png')] bg-[#2a1f0b] md:bg-none md:bg-[#2a1f0b]">
        <div className="h-full w-full md:w-1/2 p-4 flex justify-center items-center text-white flex-col">
          <Label Title="Sign up" Extra="Create new account"/>
          <Input 
            svg={<svg height="34" viewBox="0 0 48 48" width="34" xmlns="http://www.w3.org/2000/svg" fill="#ff9604" className='opacity-70 m-2'><path d="M24 4c-11.05 0-20 8.95-20 20s8.95 20 20 20 20-8.95 20-20-8.95-20-20-20zm0 6c3.31 0 6 2.69 6 6 0 3.32-2.69 6-6 6s-6-2.68-6-6c0-3.31 2.69-6 6-6zm0 28.4c-5.01 0-9.41-2.56-12-6.44.05-3.97 8.01-6.16 12-6.16s11.94 2.19 12 6.16c-2.59 3.88-6.99 6.44-12 6.44z"/><path d="M0 0h48v48h-48z" fill="None"/></svg>} 
            placeholder="Name" 
          ...
          <div className='m-2 h-12 w-[70%] shadow-[rgba(0,0,15,0.5)_5px_5px_5px_0px]'>
            <button className='bg-[#ff9604] h-full w-full rounded-md font-mono text-lg focus:bg-[#8f5e2d]' onClick={() => {
              const success = signupInput.safeParse({username: email, password: password, name: name})
              if(!success || password.length < 5 || name.length < 1){
                setError(true);
              }else{
                setError(false)
